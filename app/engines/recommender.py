...
Flask/DB에 의존하지 않는 순수 Python 모듈이다. Claude API 키가 없는 환경(현재)에서는
8.3절에 정의된 "규칙 기반 폴백"(카테고리 매칭 + 거리 + 관심사 가중치)을 유일한 스코어링
경로로 사용한다. 후보는 이미 검증된 PLACE 레코드에서만 고르므로 환각(존재하지 않는
